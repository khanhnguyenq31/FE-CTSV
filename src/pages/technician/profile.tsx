...
import { createProfileFormData, prepareProfilePayload } from "../../utils/profilePayload";
...
export default function ProfilePage({ messageApi }: { messageApi: any }) {
  ...
  const handleUpdateStudent = async (values: any, avatarFile: File | null) => {
    ...
    setEditLoading(true);
    try {
      const commonData = prepareProfilePayload(values);

      let res;
      if (avatarFile) {
        const formData = createProfileFormData(commonData, avatarFile);
